...
class DBResultStorage:
    ...
    def add_signatures(self, entries):
        ...
        for chunk in chunks(entries, size=1000):
            with self.database.session_scope() as session:
                ...
                files = query.filter(
                            self._by_path_and_hash(list(index.keys()))).all()
    ...
    def add_template_matches(self, entries, override=False):
        ...
        for chunk in chunks(entries, len(entries)):
            ...
            with self.database.session_scope() as session:
                ...
                query = session.query(Files).options(
                            joinedload(Files.templatematches))
                files = query.filter(
                                self._by_path_and_hash(
                                        list(index.keys()))).all()
                ...
                for file in files:
                    ...
                    new_tm = self._create_template_matches(
                                    file, tm, file.templatematches)
                    if len(new_tm) > 0:
    ...
    def add_matches(self, entries):
        ...
        for chunk in chunks(entries, size=1000):
            with self.database.session_scope() as session:
                ...
                matches = self._matches_by_path_and_hash(
                                    session, list(index.keys())).all()
                ...
                file_index = {
                                (file.file_path, file.sha256): file
                                for file in files}
                ...
                for (path_1,
                     sha256_1,
                     path_2,
                     sha256_2), distance in index.items():
    ...
    def _template_matches_ids(session, *files):
        ...
        template_matches = itertools.chain(*(
                                    file.templatematches
                                    or () for file in files))
    ...
    def _delete_file_scenes(session, *files):
        ...
        session.query(Scene).filter(Scene.id.in_(
                                existing_scene_ids)
                                ).delete(
    ...
    def _delete_file_template_matches(session, *files):
        """Delete all scenes associated with the given files."""
        existing_template_matches_ids = DBResultStorage._template_matches_ids(
                                                                        *files)

        (session
            .query(Templatematches)
            .filter(
                    Templatematches.id.in_(
                        existing_template_matches_ids)).delete(
                                    synchronize_session="fetch"))
    ...
    def _filter_unique_templates(old_templates, new_templates):
        ...
        filtered = []
        seen = dict({
                f'{element.file_id}{element.template_name}': True
                for element in old_templates})
    ...
    def _matches_by_path_and_hash(session, file_identifiers):
        ...
        tuple_filter = tuple_(
            query_file.file_path, query_file.sha256,
            match_file.file_path, match_file.sha256
        ).in_(file_identifiers)

        return session.query(Matches). \
    ...
    def _files_for_matches(session, connections):
        ...
        file_identifiers = set(
                            DBResultStorage
                            ._matches_file_identifiers(connections)
                            )
        existing_files = (session
                          .query(Files)
                          .filter(
                              DBResultStorage._by_path_and_hash(
                                                    file_identifiers)).all())
    ...
    def _update_exif(entity, exif):
        entity.General_FileExtension = exif.get(
                                                'General_FileExtension',
                                                entity.General_FileExtension)
        entity.General_Format_Commercial = exif.get(
                                                'General_Format_Commercial',
                                                entity
                                                .General_Format_Commercial)
        entity.General_FileSize = exif.get(
                                                'General_FileSize',
                                                entity.General_FileSize)
        entity.General_Duration = exif.get(
                                                'General_Duration',
                                                entity.General_Duration)
        entity.General_OverallBitRate_Mode = exif.get(
                                                'General_OverallBitRate_Mode',
                                                entity
                                                .General_OverallBitRate_Mode)
        entity.General_OverallBitRate = exif.get(
                                                'General_OverallBitRate',
                                                entity.General_OverallBitRate)
        entity.General_FrameRate = exif.get(
                                                'General_FrameRate',
                                                entity.General_FrameRate)
        entity.General_FrameCount = exif.get(
                                                'General_FrameCount',
                                                entity.General_FrameCount)
        entity.General_Encoded_Date = exif.get(
                                                'General_Encoded_Date',
                                                entity.General_Encoded_Date)
        entity.General_File_Modified_Date = exif.get(
        ...
        entity.General_File_Modified_Date_Local = exif.get(
                                            ...
                                            entity
                                            .General_File_Modified_Date_Local)
        entity.General_Tagged_Date = exif.get(
                                                'General_Tagged_Date',
                                                entity.General_Tagged_Date)
        entity.Video_Format = exif.get(
                                        'Video_Format',
                                        entity.Video_Format)
        entity.Video_BitRate = exif.get(
                                        'Video_BitRate',
                                        entity.Video_BitRate)
        entity.Video_InternetMediaType = exif.get(
                                                'Video_InternetMediaType',
                                                entity.Video_InternetMediaType)
        entity.Video_Width = exif.get(
                                        'Video_Width',
                                        entity.Video_Width)
        entity.Video_Height = exif.get(
                                        'Video_Height',
                                        entity.Video_Height)
        entity.Video_FrameRate = exif.get(
                                            'Video_FrameRate',
                                            entity.Video_FrameRate)
        entity.Audio_Format = exif.get(
                                        'Audio_Format',
                                        entity.Audio_Format)
        entity.Audio_SamplingRate = exif.get(
                                            'Audio_SamplingRate',
                                            entity.Audio_SamplingRate)
        entity.Audio_Title = exif.get(
                                        'Audio_Title',
                                        entity.Audio_Title)
        entity.Audio_BitRate = exif.get(
                                        'Audio_BitRate',
                                        entity.Audio_BitRate)
        entity.Audio_Channels = exif.get(
                                        'Audio_Channels',
                                        entity.Audio_Channels)
        entity.Audio_Duration = exif.get(
                                        'Audio_Duration',
                                        entity.Audio_Duration)
        entity.Audio_Encoded_Date = exif.get(
                                            'Audio_Encoded_Date',
                                            entity.Audio_Encoded_Date)
        entity.Audio_Tagged_Date = exif.get(
                                            'Audio_Tagged_Date',
                                            entity.Audio_Tagged_Date)
